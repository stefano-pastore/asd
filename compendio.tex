...
\paragraph*{}Tipicamente questo costrutto è impiegato per modificare l'iter strettamente sequenziale dell'algoritmo e coinvolge la valutazione della veridicità di una condizione al fine di poter discriminare quale ramo di computazione intraprendere.
\begin{lstlisting}[caption={Esempio di \texttt{IF-THEN-ELSE}},label={lst:ifthenelse_algo}]
IF P(n) THEN
	<istruzione1>
	<istruzione2>
	...
...
ELSE
	...
	<istruzioneN>
\end{lstlisting}
È da chiarire che nell'esempio \ref{lst:ifthenelse_algo} sopra riportato \texttt{P(n)} non rappresenta una chiamata a funzione, bensì la valutazione del predicato \texttt{P} sull'input \texttt{n}. Proprio come lo schema in figura \ref{fig:ifthenelse} rende evidente, la stima del limite superiore asintotico di un blocco \texttt{IF-THEN-ELSE} è dato $O(\max\{f(n), g(n)\})$.

\begin{figure}[hb]
	\centering
	\begin{tikzpicture}[node distance = 3.5cm, auto]
		\node[decision] (cond) {condizione};
		\node[block, below left of=cond, label={180:$O(f(n))$}] (branchA) {Blocco A};
		\node[block, below right of=cond,label={360:$O(g(n))$}] (branchB) {Blocco B};
		
		\path[line] (cond) -| node [near end, left=2pt] {\texttt{true}}(branchA);
		\path[line] (cond) -| node [near end, right=2pt] {\texttt{false}}(branchB);
		
	\end{tikzpicture}
	\caption{Schematizzazione del costrutto \texttt{IF-THEN-ELSE}.}
	\label{fig:ifthenelse}
\end{figure}
Infatti consideriamo un esempio pratico come quello seguente:
\begin{lstlisting}[caption={Esempio di \texttt{IF-THEN-ELSE}},label={lst:ifthenelse2_algo}]
IF A[1][i] = 0 THEN
	FOR i = i TO n
		FOR j = 1 TO n
			A[i][j] = 0
ELSE
	FOR i = 1 TO n
		A[i][i] = 1
\end{lstlisting}
Volendo fare una stima ad occhio del frammento di codice in questione possiamo certamente dire che all'interno del ramo \texttt{IF} i due cicli \texttt{FOR} innestati costano $O(n^2)$, mentre il ciclo del ramo \texttt{ELSE} è stimato a $O(n)$. Pertanto $T(n) = \max\{O(n^2), O(n)\} = O(n^2)$.
\pagebreak
\subsubsection{Costrutto \texttt{FOR}}I cicli sono costrutti che consentono di automatizzare la ripetizione di certe operazioni a patto che una certa condizione specificata permanga vera.
\begin{figure}[hb]
	\centering
	\begin{tikzpicture}[node distance = 2.5cm, auto]
	\node[block, label={360:$O(1)$}] (init) {inizializza};
	\node[decision, below of=init, label={360:$O(1)$}] (cond) {condizione};
	\node[block, below of=cond, label={360:$O(1)$}] (corpo) {corpo del ciclo};
	\node[block, below of=corpo,label={360:$O(1)$}] (reinit) {re-inizializza};
	
	\path[line] (init) -- (cond);
	\path[line] (cond) -- (corpo);
	\path[line] (corpo) -- (reinit);
	\path[line] (reinit.west) -- (cond.west);
...
\subsubsection{Costrutto \texttt{WHILE}}
\subsubsection{Blocchi sequenziali}
\subsubsection{Chiamata a funzione}
\subsubsection{Chiamata a funzione ricorsiva}
\paragraph*{} Supponiamo di dovere analizzare la complessità del seguente algoritmo:
\begin{lstlisting}[caption={Esempio di algoritmo ricorsivo},label={lst:rec_algo}]
RecAlgo(n)
	<istruzione1>
	<istruzione2>
	...
	...
	<istruzioneN>
\end{lstlisting}
e sia $z(n)$ la funzione che prende in ingresso l'input della chiamata corrente, lo trasforma e fa da input per la chiamata successiva. Come si fa ad arrivare ad una forma chiusa che dia un'idea della crescita asintotica dell'algoritmo?
\paragraph*{}Si utilizzano le \textbf{equazioni di ricorrenza}, che sono un metodo matematico per descrivere come una funzione distribuisce il suo input $N$ alle sotto-chiamate e quante operazioni ad ogni iterazione ricorsiva l'algoritmo esegue sulla base del suo rispettivo input.
\paragraph{La funzione fattoriale} La funzione $n!$ è detta \textbf{fattoriale} ed è definita come il prodotto degli interi che vanno da $1$ ad $n$. Posta sotto forma di equazione di ricorrenza, essa ha questa definizione:
\[n! = \left\{
\begin{array}{lr}
n\cdot (n-1)& \text{ se } n > 1\\ \rowcolor{white}
1 & \text{ se } n = 1
\end{array}
\right.
\]
che, posta in termini di funzione asintotica, possiamo riscrivere come:
\[T(n) = \left\{
\begin{array}{lr} \rowcolor{white}
\Theta(1) + T(n-1) & \text{ se } n > 1\\ 
\Theta(1) & \text{ se } n = 1
\end{array}
\right.
\]
Ma a questo punto come la trovo la forma chiusa di questa equazione di ricorrenza? Possiamo facilitarci il compito ricorrendo ad un albero di ricorrenza. Graficamente ogni cerchio corrisponde ad una chiamata alla funzione, all'interno ci poniamo il numero di operazioni che quella chiamata effettua a partire dalla definizione $T(n)$ sino ad arrivare al caso base. Esemplifichiamo con un esempio.
\paragraph{Esempio} Trovare la crescita asintotica di una funzione ricorsiva, avente la seguente equazione di ricorrenza:
\[T(n) = \left\{
\begin{array}{lr} 
3T\left(\dfrac{n}{4}\right) + n & \text{ se } n > 1\\ \rowcolor{white}
1 & \text{ se } n = 1
\end{array}
\right.
\]
...
\begin{figure}[hb]
	\centering
	\begin{forest}
		[n, for tree={circle, draw, math content, s sep=.4em}
		[\frac{n}{4}, name=a
		[\frac{n}{4^2}, name=d
		[\dots
		[,phantom] 
